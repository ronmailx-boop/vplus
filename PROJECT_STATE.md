...
## Current Focus
**הבנייה המלאה הושלמה ומוזגה ל-main.** האפליקציה כוללת: רשימות/פריטים מלאים, קטגוריות אוטומטיות, היסטוריה וסטטיסטיקה עם גרפים, Autocomplete/מחירון, ייבוא (טקסט/לוח/Excel/קול), גרירה-לסידור, נעילה, בחירה מרובה, הדפסה, PWA מלא (אייקונים/manifest/sw offline) — הכל ללא Firebase, ללא מודול פיננסי, ללא הגדרות/פוש/גוגל לוגין/סריקת קבלה, לפי ההחלטות שנקבעו.

**פערים ידועים מול shopping-list המקורי** (נשמטו במודע, ניתן להוסיף אם יתבקש): מצב קומפקטי (גובה כרטיס), חיוב חודשי קבוע + שורת "נדחה לחודש הבא", ניהול קטגוריות מותאמות אישית (היה בתוך ההגדרות המוחרגות), עיצוב פיקסל-פרפקט מול ה-HTML/CSS המקורי (הפלטה/גופנים/מבנה זהים ברוח אך לא שכפול מדויק שורה-שורה).
